...
static void do_action(vtparse_t *parser, vtparse_action_t action, char ch)
{
    ...
    switch(action) {
        ...
        case VTPARSE_ACTION_ESC_DISPATCH:
            if (parser->cb) parser->cb(parser, action, ch);
            parser->ch = ch;
            parser->processed = 1;
        ...
        default:
            if (parser->cb) parser->cb(parser, VTPARSE_ACTION_ERROR, 0);
    }
}
...
int vtparse(vtparse_t *parser, unsigned char *data, int len)
{
    int i;
    parser->ch = 0;
    parser->processed = 0;
    ...
    for(i = 0; i < len; i++)
    {
        ...
        if (parser->state == VTPARSE_STATE_ESCAPE && ch == 'O') parser->ch2 = ch; // for F1..F4
        if (parser->state == VTPARSE_STATE_GROUND) parser->ch2 = 0;
